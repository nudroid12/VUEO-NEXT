...
class PluginStore(context: Context) {
    ...
    fun pluginsEnabled(): Boolean = prefs.getBoolean(KEY_PLUGINS_ENABLED, true)

    fun setPluginsEnabled(enabled: Boolean) {
        prefs.edit().putBoolean(KEY_PLUGINS_ENABLED, enabled).apply()
    }

    fun isRepositoryEnabled(repository: PluginRepositoryDescriptor): Boolean =
        prefs.getBoolean(repositoryEnabledKey(repository.manifestUrl), true)

    fun setRepositoryEnabled(repository: PluginRepositoryDescriptor, enabled: Boolean) {
        prefs.edit().putBoolean(repositoryEnabledKey(repository.manifestUrl), enabled).apply()
    }

    fun tmdbApiKey(): String = prefs.getString(KEY_TMDB_API_KEY, "").orEmpty().trim()

    fun setTmdbApiKey(apiKey: String) {
        prefs.edit().putString(KEY_TMDB_API_KEY, apiKey.trim()).apply()
    }

    suspend fun seedDevelopmentDefaultsIfNeeded() {
        val revision = prefs.getInt(KEY_DEV_DEFAULTS_REVISION, 0)
        if (revision >= DEV_DEFAULTS_REVISION) return

        DEVELOPMENT_DEFAULT_MANIFESTS.forEach { manifestUrl ->
            runCatching { PluginRepositoryClient.fetch(manifestUrl) }
                .onSuccess(::upsert)
        }

        prefs.edit().putInt(KEY_DEV_DEFAULTS_REVISION, DEV_DEFAULTS_REVISION).apply()
    }
    ...
    fun repositories(): List<PluginRepositoryDescriptor> {
        val raw = prefs.getString(KEY_REPOSITORIES_JSON, null) ?: return emptyList()
        return runCatching {
            val array = JSONArray(raw)
            (0 until array.length())
                .mapNotNull { array.optJSONObject(it)?.toRepository() }
    ...
    fun remove(manifestUrl: String) {
        saveRepositories(repositories().filterNot { it.manifestUrl == manifestUrl })
        prefs.edit().remove(repositoryEnabledKey(manifestUrl)).apply()
    ...
    fun setProviderEnabled(
        ...
        prefs.edit().putBoolean(providerKey(repository, provider), enabled).apply()
    }

    fun totalProviderCount(): Int = repositories().sumOf { it.providers.size }

    fun enabledProviderCount(): Int = repositories()
        .filter(::isRepositoryEnabled)
        .sumOf { repository ->
            repository.providers.count { provider -> isProviderEnabled(repository, provider) }
        }

    private fun saveRepositories(repositories: List<PluginRepositoryDescriptor>) {
        val array = JSONArray()
        repositories.forEach { array.put(it.toJson()) }
        prefs.edit().putString(KEY_REPOSITORIES_JSON, array.toString()).apply()
    }

    private fun repositoryEnabledKey(manifestUrl: String): String =
        "repository_enabled:$manifestUrl"

    private fun providerKey(
        repository: PluginRepositoryDescriptor,
        provider: PluginProviderDescriptor,
    ): String = "provider:${repository.manifestUrl}:${provider.id}"
    ...
    companion object {
        ...
        private const val KEY_DEV_DEFAULTS_REVISION = "plugin_dev_defaults_revision"
...
private fun PluginRepositoryDescriptor.toJson(): JSONObject = JSONObject().apply {
...
private fun PluginProviderDescriptor.toJson(): JSONObject = JSONObject().apply {
    put("id", id)
    put("name", name)
    put("description", description)
    put("version", version)
...
private fun JSONObject.toRepository(): PluginRepositoryDescriptor? {
    val manifestUrl = optString("manifestUrl").takeIf { it.isNotBlank() } ?: return null
    val baseUrl = optString("baseUrl").takeIf { it.isNotBlank() } ?: return null
    ...
    return PluginRepositoryDescriptor(
        ...
        providers = (0 until array.length())
            .mapNotNull { array.optJSONObject(it)?.toStoredProvider() },
    )
}
